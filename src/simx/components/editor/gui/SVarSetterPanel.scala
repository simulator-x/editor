  ...
  val addButton = new Button("+"){
    ...
    reactions += {
      case ButtonClicked(source) if source == this =>
        ...
        val importRequired = man.runtimeClass.getPackage != null
        val shortClassName =
        ...
        val dr = new DynamicReloader[SVarSetterGeneratorBase] (
          ...
          Some(SVarSetterPanel.generateClassTemplateFor(shortClassName, typeName, importRequired)),
...
object SVarSetterPanel {
  ...
  def generateClassTemplateFor(className: String, typeName: String, importRequired: Boolean = true): String = {
...
import scala.swing._""" + (if(importRequired) "\nimport " + typeName + "\n" else "") + """
